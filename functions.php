 ...
 */
define( 'INSPIRY_THEME_VERSION', '2.3.1' );
...
/*-----------------------------------------------------------------------------------*/
/*	Meta Box and Related Extensions
/*-----------------------------------------------------------------------------------*/
    ...
    if(!function_exists('load_theme_scripts')){
        function load_theme_scripts(){
            if (!is_admin()) {
                ...
                // Custom Script
                wp_register_script( 'custom', $java_script_url.'custom.js', array('jquery'), INSPIRY_THEME_VERSION, true );
                ...
                if( google_map_needed() ){
                    ...
                    $map_query_args = array(                    // default map query parameters
                        'sensor' => urlencode( 'true' ),
                    );
                    if ( defined( 'ICL_LANGUAGE_CODE' ) && ( get_option('theme_map_localization') == 'true' ) ) {       // localization support
                        $query_args['language'] = urlencode( ICL_LANGUAGE_CODE );
                    }
                    wp_enqueue_script( 'google-map-api', esc_url_raw( add_query_arg( $map_query_args,  '//maps.google.com/maps/api/js' ) ) );
                    wp_enqueue_script( 'google-map-info-box', esc_url_raw( $java_script_url . 'infobox.js' ), array( 'google-map-api' ), '1.1.9' );
                    wp_enqueue_script( 'google-map-marker-clusterer', esc_url_raw( $java_script_url . 'markerclusterer.js' ), array( 'google-map-api' ), '2.1.1' );
                }
                ...
                if ( is_page_template( 'template-submit-property.php' ) ) {
                    ...
                    wp_register_script( 'property-submit', $java_script_url.'property-submit.js', array( 'jquery', 'jquery-ui-sortable', 'plupload' ), INSPIRY_THEME_VERSION, true );
                ...
                if ( is_page_template( 'template-edit-profile.php' ) ) {
                    wp_enqueue_script( 'plupload' );
                    wp_register_script( 'edit-profile', $java_script_url.'edit-profile.js', array( 'jquery', 'plupload' ), INSPIRY_THEME_VERSION, true );
